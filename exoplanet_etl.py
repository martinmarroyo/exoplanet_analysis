"""
A script to scrape data from the Extrasolar Planets Encyclopaedia (http:://www.exoplanet.eu)
...
logging.basicConfig(
    filename="log_exoplanet_etl.log",
    encoding="UTF-8",
    level=logging.INFO,
...
engine = sqlalchemy.create_engine(
    f"postgresql+psycopg2://{conf['USER']}"
...
latest_date = pd.read_sql(
    """
    SELECT MAX(updated::DATE) AS updated 
...
if inserts["updated"].max() > latest_date["updated"].max():
    try:
        with engine.begin() as conn:
            ...
            conn.execute(
                text("REFRESH MATERIALIZED VIEW exoplanet.vw_current_star_detail;")
            )
            conn.execute(
                text("REFRESH MATERIALIZED VIEW exoplanet.vw_current_exoplanet_detail;")
            )
            conn.execute(
                text("REFRESH MATERIALIZED VIEW exoplanet.vw_exoplanet_catalog_data;")
            )
